...
This is essential when multiple people are working on the same code, as code quickly becomes complex and hard to easily understand.
Typing makes it easy at a quick glance to see what some function does.
Documentation gives more insight into the inner workings of each component.
We enforced checks of these every time one of us attempts pushes to main.
The code could then only be pushed if it passed all tests.
Additionally, the code had to pass all units-tests.
## Version control
...
This is because it is almost impossible to test every scenario possible we test like these.
